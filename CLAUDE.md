...
tests/test-ui.sh                      # UI gates: gsd-ui design/layout/shots (stub Playwright + local http server), doctor T080/T018, bootstrap --no-ui
...
- **`lib/ui.sh`** — the UI quality gates (`docs/ui-quality-plan.md`), one definition for `gsd-flow-next` (steps design-system, layout, screenshots), the guard (rule 5, `ui_gates = strict`), doctor (T080) and `gsd-ui`. Artifacts: `.planning/design/DESIGN.md` (stub marker until filled), `<P>-LAYOUT.md` (`sketch:` = a `/gsd-sketch` folder whose README has a `winner:`, `pages:`), `<P>-SHOTS.md` (PNGs in `<P>-SHOTS/`, git-ignored). `ui_gates = warn` (default) / `strict` / `off`. Bootstrap never creates `.planning/` (it would flip `/gsd-ingest-docs` into merge mode); the stub is made by the flow step.
...
- The phase-flow artifact list (`<P>-CONTEXT.md`, `<P>-REVIEWS.md`, `<P>-REVIEW.md`, `<P>-UI-REVIEW.md`, `<P>-LAYOUT.md`, `<P>-SHOTS.md`, …) is shared by `gsd-flow-next`, the guard's `flow = strict` and `ui_gates = strict` rules, and gsd-doctor's T040 report. Change them together.
